...
lblInject = QtBind.createLabel(gui, 'Inject Packets to Client/Server through bot, or analyze the packets you need.', 6,
                               10)
...
def btnInjectPacket(IProxySend):
    ...
    if strOpcode and strData:
        ...
        log("Plugin: Injecting packet...\n(Opcode) 0x" + '{:02X}'.format(opcode) + " (Data) " + (
            "None" if not packet else ' '.join('{:02X}'.format(x) for x in packet)))
...
def handle_silkroad(opcode, data):
    if cbxShowClient:
        if CanShowPacket(opcode):
            log("Client: (Opcode) 0x" + '{:02X}'.format(opcode) + " (Data) " + (
...
def handle_joymax(opcode, data):
    if cbxShowServer:
        if CanShowPacket(opcode):
            log("Server: (Opcode) 0x" + '{:02X}'.format(opcode) + " (Data) " + (
                "None" if not data else ' '.join('{:02X}'.format(x) for x in data)))
    return True
